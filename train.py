...
from datasets import build_dataset, test_transforms, train_transforms
from engine import evaluate, train_one_epoch, train_one_epoch_w_accum
...
def get_args_parser():
    parser = argparse.ArgumentParser(
        "Transformer-based visual grounding", add_help=False
    )
    ...
    parser.add_argument(
        "--bert_output_layers", default=4, type=int, help="the output layers of Bert"
    )
    parser.add_argument(
        "--max_query_len",
        default=40,
...
def main(args):
    ...
    if args.eval:
        print(epoch_eval)
        test_stats, test_acc, test_time = epoch_eval(
            model,
            criterion,
            postprocessor,
            data_loader_val,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        "VLTVG training script", parents=[get_args_parser()]
    )
